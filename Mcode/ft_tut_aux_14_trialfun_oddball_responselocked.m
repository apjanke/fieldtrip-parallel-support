 function [trl, event] = ft_tut_aux_14_trialfun_oddball_responselocked(cfg)
...
for istim = 1:length(stim_indx);
  ...
  if value(stim_indx(istim)) == 2
    ...
    newtrl   = [ sample(rsp_indx(trl_rsp_indx))+pretrig sample(rsp_indx(trl_rsp_indx))+posttrig pretrig rsp RT];
    trl      = [ trl; newtrl ];
  end
